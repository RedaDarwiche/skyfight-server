...
function spawnNPC(rarity, slotIdx) {
    ...
    npcs[id] = {
        ...
        aggroTarget: null  // only chase/attack the player who hit them
    };
    ...
    npcAIInterval = setInterval(() => {
        try {
            for (const npcId in npcs) {
                ...
                // Only chase/attack the player who aggroed us
                let target = null;
                if (npc.aggroTarget && players[npc.aggroTarget] && (players[npc.aggroTarget].hp || 0) > 0) {
                    target = { id: npc.aggroTarget, ...players[npc.aggroTarget] };
                } else {
                    // Clear dead/gone aggro target
                    ...
                    if (npc.attackTimer >= 5) {
                        ...
                        if (p && (p.hp || 0) > 0) {
                            ...
                            if (d < npc.attackRange) {
                                players[target.id].hp = Math.max(0, (players[target.id].hp || 100) - npc.attackDmg);
                                io.to(target.id).emit('playerHit', { targetId: target.id, damage: npc.attackDmg, attackerId: npc.id });
                            }
                        }
                    }
                } else {
...
io.on('connection', (socket) => {
    ...
    socket.on('playerDied', (data) => {
        try {
            ...
            if (data.killerId && players[data.killerId]) {
                ...
                // Broadcast killer's new kill count so all leaderboards update immediately
                io.emit('killsUpdate', { playerId: data.killerId, kills: players[data.killerId].kills });
            }
    ...
    socket.on('npcHit', (data) => {
        try {
            ...
            // Set aggro on the attacker so NPC fights back
            npc.aggroTarget = socket.id;
            io.emit('npcMoved', { npcId: npc.id, x: npc.x, y: npc.y, angle: npc.angle, hp: npc.hp });
